...
import webservices.AuthenticationRecource;
    ...
    public void filter(ContainerRequestContext requestCtx) {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            try {
                // Validate the token
                JwtParser parser =
                        Jwts.parser().setSigningKey(AuthenticationRecource.key);
                Claims claims = parser.parseClaimsJws(token).getBody();
                String user = claims.getSubject();
                msc = new MySecurityContext(Werknemer.getWerknemerByNaam(user), scheme);
